...
| `analyze_job_fit_report` | 구현됨 MVP | `job_id`, `target_role`, `known_skills`, 선택 입력 `duty_description_text`를 받아 `preparation_items`, `knowledge_gaps`, `ncs_mapping`, `evidence_links`, `verification_notes`를 생성한다. 기관 분석은 아직 자동 연결하지 않는다. |
