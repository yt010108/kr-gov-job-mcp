...
유형을 선택한 뒤 필요한 정보만 입력하면 `public_job_career_coach`가 공고 검색, 상세 조회,
평균보수, 적합도, 기관 분석, 면접, STAR를 자동으로 연결합니다. 결과는 공고 탐색·지원 준비·
면접 준비 중 하나의 한 화면 대시보드로 반환됩니다.

공고 탐색 화면에는 최대 3개 공고의 D-day, 기관 평균보수, 적합 근거, 보완 역량, 오늘 할 일,
원문 링크가 표시됩니다. 일부 조회는 `partial_success`로 결과와 경고를 함께 보존하고,
결과가 없으면 `no_results`를 반환합니다.

> 평균보수는 기관 전체 직원 기준이며 신입 초봉이나 채용 제시 연봉이 아닙니다. 실행 계획만
