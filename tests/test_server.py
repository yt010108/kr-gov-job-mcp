...
def test_server_list_tools_command_outputs_registered_tools(capsys) -> None:
    ...
    assert payload["tools"][5]["name"] == "get_institution_average_salary"
    assert payload["tools"][6]["name"] == "health_check"
    assert payload["tools"][7]["name"] == "lookup_job_alio_codes"
    assert payload["tools"][8]["name"] == "lookup_region_codes"
    assert payload["tools"][9]["name"] == "map_ncs_competencies"
    assert payload["tools"][10]["name"] == "prepare_application_strategy"
    assert payload["tools"][11]["name"] == "prepare_institution_interview"
    assert payload["tools"][12]["name"] == "public_job_career_coach"
    assert payload["tools"][13]["name"] == "resolve_ncs_code"
    assert payload["tools"][14]["name"] == "search_public_jobs"
