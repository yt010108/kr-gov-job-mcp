...
| `html` | 원문 HTML | 기관 채용 페이지, Cleaneye/ALIO 공시 본문 |
...
| `career_page` | 기관 채용 페이지 |
| `press_release` | 기관 보도자료 |
...
| 구분 | 필드 |
| --- | --- |
| 안정 필드 | `source_url`, `final_url`, `status_code`, `content_type`, `title` |
| 자주 비는 필드 | 본문 preview, 첨부 후보, 접수 링크 후보 |
| 전용 필드 | `page_type`, 링크 후보 목록 |
| 임시 정규화 후보 | `page_title`, `page_type`, `attachment_candidates`, `apply_candidates` |

전용 채용 플랫폼이나 JavaScript 렌더링 페이지는 정적 HTML만으로 본문이 비어 있을 수 있다.
자동 접수나 로그인 흐름은 raw inventory 범위 밖이다.

### 기관 보도자료

| 구분 | 필드 |
| --- | --- |
| 안정 필드 | 목록 URL, 제목, 상세 URL, 등록일 |
| 자주 비는 필드 | 본문 preview, 첨부 후보, 키워드 매칭 결과 |
| 전용 필드 | `postSeq`, 상세 링크 패턴, 키워드 필터 |
| 임시 정규화 후보 | `evidence_source`, `matched_keywords`, `excerpt` |

보도자료는 기관 사업 방향의 보조 근거다. 최근성, 원문 URL, 본문 excerpt를 함께 저장해야 한다.
