...
`Situation:`, `Task:`, `Action:`, `Result:`처럼 항목을 명시하면 해당 원문을 그대로 분류한다.
라벨이 없는 문장은 한 STAR 항목에만 대응하는 단서가 있을 때만 후보로 분류한다. 여러 항목으로
해석할 수 있거나 단서가 없는 문장은 빈 항목에 순서대로 배치하지 않고
`unclassified_excerpts`에 보존한다.

## 출력 계약

- `star`: 각 STAR 항목의 `source_excerpts`, `status`, 작성 가이드
- `unclassified_excerpts`: 하나의 STAR 항목으로 안전하게 분류할 수 없는 사용자 원문
