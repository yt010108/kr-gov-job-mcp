...
| `warnings` | 검색 경고 목록 |
| `diagnostics` | 검색 결과가 0건일 때만 포함되는 진단 정보 |
| `diagnostics.no_result` | 0건 진단 여부 |
| `diagnostics.applied_filters` | 실제 적용된 검색 필터 |
| `diagnostics.resolved_filters` | 자연어에서 코드로 변환된 필터 |
| `diagnostics.keyword_scope` | `keyword`가 어떤 검색 범위에 쓰였는지 설명 |
| `diagnostics.possible_causes` | 0건이 나온 가능한 이유 목록 |
| `suggested_next_queries` | 검색 결과가 0건일 때만 포함되는 재검색 제안 목록 |
| `suggested_next_queries[].tool` | 다음에 호출하면 좋은 도구 이름 |
| `suggested_next_queries[].reason` | 해당 재검색을 제안하는 이유 |
| `suggested_next_queries[].arguments` | 재검색에 사용할 인자 |
...
- 결과가 0건이면 기존 응답 구조를 유지하면서 `diagnostics`와 `suggested_next_queries`를 추가한다.
- `keyword`는 현재 잡알리오 채용공고 제목 검색어로 전달되므로, 기관명처럼 보이는 검색어는
  기관 코드 조회를 제안한다.
- 현재 접수 중인 공고만 조회하다가 0건이 나오면 `ongoing_only=false` 재검색을 제안한다.
