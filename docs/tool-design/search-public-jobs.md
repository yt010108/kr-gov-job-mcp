...
| `ncs_code` | Job-ALIO NCS 코드 필터. 자연어 직무명/NCS명은 `resolve_ncs_code`로 먼저 해석한 확정 코드만 입력 |
| `region` | 자연어 지역명 |
...
| `warnings` | 검색 경고 목록 |
| `diagnostics` | 결과가 0건일 때만 반환하는 검색 진단 정보. 결과가 있으면 `null` |
| `diagnostics.reason` | 현재 값은 `no_results` |
| `diagnostics.keyword_search_scope` | `keyword`가 적용되는 검색 범위 설명 |
| `diagnostics.recommended_next_calls` | 재검색 또는 코드 조회에 바로 사용할 수 있는 MCP 도구 호출 제안 |
...
- `institution_code`, `ncs_code`는 Job-ALIO 코드 입력 필드이며, 자연어 기관명이나 직무명은
  기관은 `lookup_job_alio_codes`, 직무는 `resolve_ncs_code`로 확인한 뒤 전달한다.
...
- 검색 결과가 0건이면 `diagnostics`에 현재 접수 중 필터 해제, 기관명/NCS 코드 조회,
  세부 필터 해제 재검색 중 해당하는 다음 호출을 제안한다. `keyword`가 기관명처럼 보이면
  기관 코드 조회를, 그 외 직무 키워드에는 NCS 코드 조회를 우선 제안한다.
