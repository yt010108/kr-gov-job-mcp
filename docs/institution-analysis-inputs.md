...
결과를 만들지 않고, ALIO, Cleaneye, 기관 홈페이지, 채용 페이지, 보도자료에서 얻은 원문
...
| 채용 페이지 | Job-ALIO `source_url` | 직무와 연결되는 현장 업무 힌트 | 채용 요구사항에서 드러나는 보완점 | 보조 근거 |
| 보도자료 | 상세 URL, 등록일 | 최근 사업, 협력, 정책 키워드 | 이슈 대응, 설명자료 | 최근성 필터 필요 |
...
| category | 용도 |
| --- | --- |
| `business_direction` | 주요사업, 성장 사업, 보도자료 기반 최근 방향 |
...
| 필드 | 예시 |
| --- | --- |
| `title` | `ALIO 주요사업`, `KISA 보도자료` |
| `source_type` | `alio_disclosure`, `cleaneye`, `press_release` |
| `url` | 원문 URL |
| `source_id` | `apbaId`, `entId`, `postSeq` 등 |
| `excerpt` | 분석 후보를 만든 원문 일부 |
| `fields` | 원본 itemNo, itemId, matched_keywords 등 |
...
보도자료 collector가 머지되면 각 collector 결과를 evidence와 signal 후보로 변환하는 adapter를
추가한다.
