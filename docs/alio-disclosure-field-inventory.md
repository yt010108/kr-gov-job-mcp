...
정기 보고서 `31501`, `7030`은 목록 JSON의 `files` 문자열과
`/item/itemReportFiles.json` 응답을 같은 attachment 모델로 정규화한다.

| 정규화 필드 | 목록 JSON `files` | `itemReportFiles.json` | 비고 |
| --- | --- | --- | --- |
| `file_no` | `101@파일명`의 `101` | `fileNo` | 다운로드 key |
| `original_name` | `101@파일명`의 `파일명` | `orcpFileNa` | 표시 파일명 |
| `download_url` | `file_no`, `disclosure_no`, `submission_no` 조합 | 동일 | `/download/file.json?f=...&d=...&s=...` |
| `disclosure_no` | 목록 row의 `disclosureNo` | `disclosureNo` | 보고서 상세 key |
| `submission_no` | 목록 row의 `submissionNo` | `submissionNo` | 제출 번호 |
| `report_form_no` | 목록 row의 `reportFormNo` | `reportFormNo` | `7030`은 `70301`처럼 세부 번호 가능 |
| `institution_id` | `apbaId` | `apbaId` | 기관 코드 |
| `file_size` | 없음 | `fileSize` | `0`도 정상 값으로 보존 |

목록 JSON의 `files`는 `fileNo@fileName|fileNo@fileName` 패턴이다. `@`처럼 fileNo가 없는
placeholder는 파일 0개로 처리한다. `itemReportFiles.json`은 같은 fileNo에 저장 경로, 저장 파일명,
파일 유형, 크기를 덧붙인 상세 메타데이터로 본다.
